...
class MakePlaylistRecommendation(APIView):
    def get(self, request, id):
        ...
        def extract_data(playlistDF, playlist_URI):
            ...
            for track in sp.playlist_tracks(playlist_URI)["items"]:
                try:
                    ...
                    ser = pd.Series([track_id, track_name, track_pop, artist_name, artist_pop, genres, danceability,
                                    energy, key, loudness, mode, speechiness, acousticness, instrumentalness, liveness, valence, tempo])
                    playlistDF = pd.concat(
                        [playlistDF, ser], axis=1, ignore_index=True)

                except:
                    continue

            playlistDF = playlistDF.transpose()
            playlistDF.columns = ['track_id', 'track_name', 'track_pop', 'artist_name', 'artist_pop', 'genres', 'danceability',
                                  'energy', 'key', 'loudness', 'mode', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo']
        ...
        def drop_duplicates(df):
            ...
            '''
            df['artists_song'] = df.apply(
                lambda row: row['artist_name']+row['track_name'], axis=1)
        ...
        def sentiment_analysis(df, text_col):
            ...
            '''
            df['subjectivity'] = df[text_col].apply(getSubjectivity).apply(
                lambda x: getAnalysis(x, "subjectivity"))
        ...
        def create_feature_set(df, float_cols):
            ...
            genre_df.columns = ['genre' + "|" +
                                i for i in tfidf.get_feature_names()]
        ...
        def generate_playlist_recos(df, features, nonplaylist_features):
            ...
            non_playlist_df['sim'] = cosine_similarity(nonplaylist_features.drop(
                'track_id', axis=1).values, features.values.reshape(1, -1))[:, 0]
            non_playlist_df_top_20 = non_playlist_df.sort_values(
                'sim', ascending=False, ignore_index=True).head(20)
        ...
        recommend = generate_playlist_recos(
            songDF, complete_feature_set_playlist_vector, complete_feature_set_nonplaylist)
        '''
