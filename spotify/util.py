...
def update_or_create_user_tokens(session_id, access_token, token_type, refresh_token, expires_in):
    ...
    if tokens:
        ...
        tokens = SpofityToken(user=session_id, access_token=access_token,
...
def extract_data(session_id, playlistDF, playlist_id):
    ...
    for track in songs['items']:
        try:
            ...
            series = pd.Series([track_id, track_name, track_pop, artist_name, artist_pop, genres, danceability,
                               energy, key, loudness, mode, speechiness, acousticness, instrumentalness, liveness, valence, tempo])
            playlistDF = pd.concat(
                [playlistDF, series], axis=1, ignore_index=True)
...
def get_recommended_tracks(session_id, playlist_id):
    ...
    recommended_songs = generate_playlist_recos(
